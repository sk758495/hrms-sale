...
class SalarySlipApiController extends Controller
{
    ...
    {
        $slip = SalarySlip::with(['user', 'employeeData.department', 'employeeData.position', 'hr'])->findOrFail($id);
    ...
    public function store(Request $request)
    {
        ...
        if (SalarySlip::where('user_id', $request->user_id)->where('month_year', $request->month_year)->exists()) {
            return response()->json([
                'status' => false,
                'message' => 'Salary slip already exists for this employee this month.'
            ], 422);
        }
